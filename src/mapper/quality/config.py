...
def load_quality_config(config_path: str | None = None) -> models.QualityConfig:
    ...
    type_cov_data = quality_section.get("type-coverage", {})
    ...
    if not (0 <= type_coverage.min_coverage <= 100):
        raise ValueError(
            f"type-coverage.min_coverage must be between 0 and 100, got {type_coverage.min_coverage}"
        )

    # Load docstring coverage config
    doc_cov_data = quality_section.get("docstring-coverage", {})
    ...
    if not (0 <= docstring_coverage.min_coverage <= 100):
        raise ValueError(
            f"docstring-coverage.min_coverage must be between 0 and 100, got {docstring_coverage.min_coverage}"
        )

    # Load param complexity config
    param_comp_data = quality_section.get("param-complexity", {})
    ...
    if param_complexity.max_parameters <= 0:
        raise ValueError(
            f"param-complexity.max_parameters must be greater than 0, got {param_complexity.max_parameters}"
        )
